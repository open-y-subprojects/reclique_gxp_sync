...
/**
 * Convert ReClique Class name to drupal node ID and revision ID.
 *
 * @MigrateProcessPlugin(
 *   id = "reclique_class"
 * )
 */
class ReCliqueClass extends ProcessPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    $className = $activityName = $source['class'];
    ...
    if ($subcategoryId = $this->config->get('program_subcategory')) {
      ...
      $activityName = isset($category_mapping_parsed[$activityName]) ? $category_mapping_parsed[$className] : $activityName;

      $activityId = $this->getActivity($activityName, $subcategoryId);
      $classId = $this->getClass($className, $activityId);
    }

    if (!$classId) {
      $message = $this->t('ReClique Migration: value="@value", title="@title", type="@type"', [
        '@value' => $value,
    ...
    // Assign the ID to the session.
    return [
      'target_id' => $classId,
      'target_revision_id' => $classId,
    ];
  }
  ...
  private function getActivity($activity_name, $subcategoryId) {
    ...
    if (count($result) > 1) {
      $msg = "Found duplicated activity name ' . $activity_name . ' within subcategory id ' . $subcategoryId . '";
    }
   ...
   *   Class node id.
   */
  private function getClass($class_name, $activityId) {
    $result = $this->nodeStorage->getQuery()
      ->condition('title', $class_name)
      ->condition('type', 'class')
      ->condition('field_class_activity.target_id', $activityId)
      ->execute();

    // Make sure there is no duplicates.
    if (count($result) > 1) {
      $this->logger->info("Found duplicated class name ' . $class_name . ' withing activity id ' . $activityId . '");
    }
